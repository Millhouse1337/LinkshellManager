-- linkshellmanager.lua (Refactored entry point)
-- All addon behavior lives in submodules under linkshellmanager/. This file
-- is now just the addon header, requires, single state-table init, config +
-- api wiring, and the ashita event registrations -- each of which delegates
-- to a module's entry function.
--
-- Module layout:
--   linkshellmanager/migrations.lua     - one-time config migrations
--   linkshellmanager/state_defaults.lua - default state table factory
--   linkshellmanager/utils.lua          - small helpers (ls_prefix, etc.)
--   linkshellmanager/commands.lua       - /lsm, /attend
--   linkshellmanager/text_parser.lua    - text_in HNM defeat / loot detector
--   linkshellmanager/render_pump.lua    - d3d_present orchestrator
--   linkshellmanager/render/poll.lua    - per-frame timer & poll dispatch
--   linkshellmanager/render/callbacks.lua - launcher callback table builder
--   linkshellmanager/render/callbacks_attendance.lua / _event.lua / _loot.lua
--                                       - the actual callback bodies.

addon.name    = 'lsm'
addon.author  = 'millhouse'
addon.version = '4.1.8'
addon.desc    = 'LSManager linkshell-attendance addon'

require('common')

-- Setup package path to include the current directory (New Att)
-- Assuming this file is in .../lsm/New Att/
...
-- Pass settings.save as a callback so the API module can persist auto-
-- cleanups (e.g. removing a pairing whose token the server rejected).
api.set_config(config.api, settings.save)
...
-- `<AshitaPath>/config/addons/lsm/<CharName>_<ServerId>/settings.lua`.
...
-- behaves as if /lsm server and /lsm link were never run — the user
...
settings.register('settings', 'lsm_config_reload', function(reloaded)
...
-- INITIALIZATION
--------------------------------------------------------------------------------
ashita.events.register('load', 'lsm_load_cb', function()
...
-- COMMAND HANDLERS
--------------------------------------------------------------------------------
-- /lsm and /attend handlers are registered inside this call.
commands.register(state, deps)

--------------------------------------------------------------------------------
-- TEXT_IN: HNM defeat-message detector (ToD Capture)
--------------------------------------------------------------------------------
ashita.events.register('text_in', 'lsm_tod_capture_cb', function(e)
...
-- D3D PRESENT
--------------------------------------------------------------------------------
ashita.events.register('d3d_present', 'lsm_present_cb', function()
