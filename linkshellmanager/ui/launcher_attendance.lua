...
-- Returns v when v is a non-empty string; otherwise the fallback. Guards
-- against the JSON-null sentinel that the addon's JSON lib decodes as a
-- truthy *table* (see callbacks_event.lua:248). Without this, downstream
-- string.format('%s', t) would print "table: 0x..." for any null field.
...
function M.draw(state, callbacks)
    ...
    do
        ...
        -- In-zone preview: lists every linkshell-roster member visible in
        -- the FFXI entity list right now, filtered by the active scope.
        -- Lets the leader see who Post Window would credit BEFORE clicking
        -- it. Compact mode hides it (single-line header layout). Non-HNM
        -- events skip it since scope filtering is forced to 'zone' and the
        -- live roster already covers the same purpose.
        if isHnmEvent and not isCompact then
            -- Warm the roster cache so we have names to intersect with.
            if state.rosterCache == nil and callbacks.on_load_roster then
                callbacks.on_load_roster(false)
            end
            local rosterNamesNow = (state.rosterCache and state.rosterCache.names) or {}

            -- Cache the scan result for 1 second to keep render fast. Scope
            -- changes invalidate immediately so the preview never lags
            -- behind a radio toggle.
            state.zonePreviewCache = state.zonePreviewCache or { ts = 0, scope = nil, rows = {} }
            local nowTs = os.time()
            local cache = state.zonePreviewCache
            if cache.scope ~= state.attendanceScopeFilter
                    or (nowTs - (cache.ts or 0)) >= 1 then
                cache.rows = attendance.list_inzone_lsmembers(rosterNamesNow)
                cache.scope = state.attendanceScopeFilter
                cache.ts = nowTs
            end

            local scopeLabel = (state.attendanceScopeFilter or 'zone'):gsub('^%l', string.upper)
            if #rosterNamesNow == 0 then
                imgui.TextDisabled('In zone now: loading roster…')
            else
                imgui.TextDisabled(string.format('In zone now (Scope=%s): %d LS member%s',
                    scopeLabel, #cache.rows, (#cache.rows == 1) and '' or 's'))
            end
            imgui.BeginChild('##attInzonePreview', { 0, 90 }, true)
            if #cache.rows == 0 then
                imgui.TextDisabled('No matching LS members visible in zone.')
            else
                for _, row in ipairs(cache.rows) do
                    imgui.Text(string.format('%s (%s/%s)', row.name, row.main, row.sub))
                end
            end
            imgui.EndChild()
        end
                ...
                if serverP and serverP.isOnBreak then
                    ...
                    local line = isCompact
                        ...
                        or string.format('%s (%s | %s/%s)%s%s',
                            r.name, safeStr(r.zone, '?'), safeStr(r.jobsMain, '?'), safeStr(r.jobsSub, '?'),
        ...
        -- Per-window DKP rate. When called with a sequence number, returns the
        -- rate captured at the moment that specific window was posted (so the
        -- frozen-window display keeps showing the original "+N DKP" suffix
        -- even after the leader changes the rate for future windows). With no
        -- argument, returns the *current* event rate — used by the live tab's
        -- per-row preview and as the default value for the next-window input.
        -- For windowed events the server reuses the dkpPerHour column as
        -- DkpPerWindow (same column, different semantic).
        local function dkp_per_window_for_active_event(seq)
            if seq and state.windowStateByEvent and state.linkedEventId then
                local entry = state.windowStateByEvent[state.linkedEventId]
                local atPost = entry and entry.dkpAtPost
                if atPost then
                    local n = tonumber(atPost[seq])
        ...
        local function render_frozen_roster(seq, snapshot)
                            ...
                            elseif result then
                                ...
                                elseif matched > 0 then
                                    ...
                                    table.insert(snapshot, {
                                        name     = safeStr(credited and credited.characterName, name),
                                        zone     = '—',
                                        jobsMain = safeStr(credited and credited.jobName, '?'),
                                        jobsSub  = safeStr(credited and credited.subJobName, '?'),
                                    })
            ...
            -- Pass the window's sequence so we read the rate captured at the
            -- time this window was posted (not the current event setting),
            -- preserving historical accuracy when the rate is changed mid-event.
            local dkpPerWindow = dkp_per_window_for_active_event(seq)
            local i = 1
            while i <= #snapshot do
                local r = snapshot[i]
                ...
                local rowLine = isCompact
                    ...
                    or string.format('%s (%s | %s/%s)%s',
                        name, safeStr(r.zone, '?'), safeStr(r.jobsMain, '?'), safeStr(r.jobsSub, '?'), dkpSuffix)
                imgui.Text(rowLine)
                imgui.SameLine()
        ...
        if not isHnmEvent then
            ...
            if active == 'inprog' then
                -- Per-tab DKP-per-window input: lets the leader change the
                -- rate that the NEXT Post Window will apply, without
                -- opening the Settings dialog. Past frozen windows keep
                -- the rate they were posted with (see dkpAtPost in
                -- windowStateByEvent, captured in callbacks_event.lua
                -- when the snapshot was frozen). Hidden until an event is
                -- linked so we have an event id to PATCH against.
                if not isCompact and state.linkedEventId and state.webEvents then
                    local linkedEv
                    for _, ev in ipairs(state.webEvents) do
                        if ev.id == state.linkedEventId then linkedEv = ev; break end
                    end
                    if linkedEv then
                        state.inprogDkpInput = state.inprogDkpInput or {}
                        local inputKey = state.linkedEventId
                        local currentRate = tonumber(linkedEv.dkpPerHour) or 0
                        -- Initialize (or re-init when the event id changes)
                        -- so the box shows the live rate by default.
                        if state.inprogDkpInput[inputKey] == nil then
                            state.inprogDkpInput[inputKey] = tostring(currentRate)
                        end
                        local p = { state.inprogDkpInput[inputKey] }
                        imgui.AlignTextToFramePadding()
                        imgui.Text('DKP per window:')
                        imgui.SameLine()
                        imgui.PushItemWidth(60)
                        if imgui.InputText(
                                string.format('##inprogDkpRate_%d', inputKey), p, 8) then
                            state.inprogDkpInput[inputKey] = p[1] or ''
                        end
                        imgui.PopItemWidth()
                        imgui.SameLine()
                        if imgui.SmallButton(
                                string.format('Apply##inprogDkpApply_%d', inputKey)) then
                            local newRate = tonumber((p[1] or ''):match('%S+'))
                            if not newRate or newRate < 0 then
                                state.lastSyncSummary = 'DKP must be a non-negative number.'
                            elseif math.abs(currentRate - newRate) < 0.0001 then
                                state.lastSyncSummary = string.format(
                                    'DKP / window already %g.', currentRate)
                            else
                                local _, errUp = api.update_event_dkp(
                                    state.linkedEventId, newRate)
                                if errUp then
                                    state.lastSyncSummary =
                                        'DKP update failed: ' .. tostring(errUp)
                                else
                                    linkedEv.dkpPerHour = newRate
                                    state.inprogDkpInput[inputKey] = tostring(newRate)
                                    state.lastSyncSummary = string.format(
                                        'DKP / window set to %g - applies to the next Post.',
                                        newRate)
                                end
                            end
                        end
                        imgui.SameLine()
                        imgui.TextDisabled('(applies to next Post only)')
                    end
                end
                render_live_roster()
            elseif type(active) == 'number'
                ...
                if not isCompact then
                    ...
                    -- Passing the active sequence pins this to the rate captured
                    -- at post time, so changing the rate mid-event doesn't
                    -- silently rewrite previously-posted windows' displayed math.
                    local snap = state.windowRosters[active]
                    local rate = dkp_per_window_for_active_event(active)
