...
function M.install(out, state, deps)
    ...
    out.on_start_and_post = function(opts)
        if not api.is_paired() then
            return 'Not paired with web. Use /lsm link <code>.'
        end
                ...
                if nextSequence then
                    ...
                    if not entry then
                        entry = {
                            ...
                            postedAt = {},
                            -- Per-window snapshot of the DKP rate at the moment the
                            -- window was posted. Lets the frozen display keep its
                            -- original "+N DKP" suffix even if the leader changes
                            -- the event's DKP/window rate mid-event for future
                            -- windows. Indexed by sequence number, value is a
                            -- positive number (or nil when no rate was known).
                            dkpAtPost = {},
                        }
                        ...
                        entry.dkpAtPost = entry.dkpAtPost or {}
                    end
                    entry.postedAt[nextSequence] = constants.format_posted_at(os.date('*t'))
                    -- Prefer the server's per-window rate (it's authoritative for
                    -- the windowed DKP math) and fall back to the live event's
                    -- cached dkpPerHour when the response shape changes.
                    local capturedRate = tonumber(rate)
                    if (not capturedRate or capturedRate <= 0) and state.webEvents then
                        for _, ev in ipairs(state.webEvents) do
                            if ev.id == state.linkedEventId then
                                capturedRate = tonumber(ev.dkpPerHour)
                                break
                            end
                        end
                    end
                    if capturedRate and capturedRate > 0 then
                        entry.dkpAtPost[nextSequence] = capturedRate
                    end
                    attendance.clear()
                    attendance.add_self()
                end
