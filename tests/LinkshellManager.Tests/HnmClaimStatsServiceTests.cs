...
public class HnmClaimStatsServiceTests
{
    ...
    private static Tod Claim(int id, int linkshellId, string monster, DateTime when, bool? claim = true) => new()
    {
        Id = id,
    ...
    // An HNM board logs its ToD under the board's AssignedMonsterName verbatim — the combined
    // "Behemoth/King Behemoth" from day 4 on — while a manual ToD or an early-day board logs
    // the bare base name. They are the same camp, so they are one slice.
    [Fact]
    public async Task MergePairHalves_CountAsOneMonster()
    {
        using var db = NewInMemoryContext();
        var now = DateTime.UtcNow;
        db.Tods.Add(Claim(1, 10, "Behemoth", now.AddDays(-1)));
        db.Tods.Add(Claim(2, 10, "Behemoth/King Behemoth", now.AddDays(-2)));
        ...
        Assert.Equal("Behemoth/King Behemoth", slice.MonsterName);
        Assert.Equal(3, slice.Count);
    }

    // The full chart roster, and the whole of it: the 12 built-in HNMs as 9 entries. The old
    // aggregation kept only the top 6 (the palette was 6 colours long), so a linkshell camping
    // the full list lost the tail of the legend and the ring never closed.
    [Fact]
    public async Task ChartsExactlyTheNineHnmEntries()
    {
        using var db = NewInMemoryContext();
        var now = DateTime.UtcNow;
        // Logged under a mix of spellings — bare halves, stronger halves, combined labels — the
        // way boards and manual ToDs actually record them.
        var logged = new[]
        {
            "Adamantoise", "Aspidochelone/Adamantoise", "Behemoth", "King Behemoth",
            "Fafnir/Nidhogg", "Nidhogg", "Cerberus", "Hydra", "Jormungand", "Khimaira",
            "Tiamat", "Vrtra"
        };
        var id = 1;
        foreach (var monster in logged)
        {
            db.Tods.Add(Claim(id++, 10, monster, now.AddDays(-1)));
        }
        await db.SaveChangesAsync();
        ...
        // Accounts for the whole ring (to the nearest point — the merged pairs take 2/12 each).
        // Truncating to a top 6 landed here at ~66%, which is what left the donut visibly open.
        Assert.Equal(100, stats.AllTime.Sum(slice => slice.Percent), 0);
        // Distinct colours all the way down, so no two monsters share a legend swatch.
