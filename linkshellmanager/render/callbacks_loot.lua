...
function M.install(out, state, deps)
    ...
    out.on_post_tod = function(captureIndex, claimed)
        ...
        if not api.is_paired() then
            cap.postError = 'Not paired with web. Use /lsm link <code>.'
            return
        end
        ...
        if cap.posted and cap.posted.todId then
            ...
            if not result then
                ...
                print(chat.header('lsm') .. 'Update claim failed: ' .. cap.postError)
                return
            end
            cap.posted.claim = (result.claim == true) or false
            print(chat.header('lsm') .. string.format('ToD claim updated: %s -> %s',
        ...
        if not result then
            cap.postError = tostring(err or 'unknown error')
            print(chat.header('lsm') .. 'Post ToD failed: ' .. cap.postError)
            return
        end
        ...
        print(chat.header('lsm') .. string.format('Posted ToD: %s (id %s, %s)',
    ...
    out.on_load_roster = function(force)
        ...
        if not api.is_paired() then
            state.rosterError = 'Not paired with web. Use /lsm link <code>.'
            return
        end
    ...
    out.on_post_loot = function(captureIndex, dropIndex)
        ...
        if not api.is_paired() then
            drop.postError = 'Not paired with web. Use /lsm link <code>.'
            return
        end
        ...
        if not result then
            ...
            print(chat.header('lsm') .. 'Post Loot failed: ' .. drop.postError)
            return
        end
