...
local function get_url(path)
    if not api.config or not api.config.baseUrl or api.config.baseUrl == '' then
        return nil, 'No web server configured. Use /lsm server <url> first.'
    end
...
-- Removes a specific pairing record from the local config by token match.
-- Called when the server rejects a request with 401/403, so a revoked or
-- expired token disappears from /lsm status without the user having to
-- manually run /lsm unlink. Saves config if a removal occurred so the
-- cleanup survives a reload.
local function purge_pairing_by_token(token)
    if not api.config or not api.config.pairings or not token or token == '' then
        return false
    end
    local removed = false
    for i = #api.config.pairings, 1, -1 do
        if api.config.pairings[i].token == token then
            table.remove(api.config.pairings, i)
            removed = true
        end
    end
    if removed and api.config_save then
        pcall(api.config_save)
    end
    return removed
end

-- Internal request worker. `override_token` lets validate_pairings() probe a
-- specific pairing's token instead of always reaching for the primary one.
-- Pass `skip_purge = true` to suppress the 401/403 auto-unpair (used by the
-- validation loop itself, which manages its own removals to keep messaging
-- sensible).
local function request_with_token(method, path, body_table, override_token, skip_purge)
    ...
    local used_token = override_token
    if not used_token then
        ...
        if primary and primary.token and primary.token ~= '' then
            used_token = primary.token
        end
    end
    if used_token and used_token ~= '' then
        headers['Authorization'] = 'Bearer ' .. used_token
    end
    if body_string then
    ...
    -- Auto-unpair on hard auth failure when the server says "this token is
    -- not valid". Revoked-from-the-web flow lands here on the next request.
    if (status == 401 or status == 403) and used_token and not skip_purge then
        purge_pairing_by_token(used_token)
    end
...
local function request(method, path, body_table)
    return request_with_token(method, path, body_table, nil, false)
end

-- ---------- Public API (unchanged) ----------

function api.set_config(cfg, save_fn)
    api.config = cfg
    if type(save_fn) == 'function' then api.config_save = save_fn end
end
...
-- Probes each known pairing's token against /api/addon/me and removes any
-- that the server now rejects with 401/403 (token revoked from the web UI,
-- token wiped from the DB, etc.). Returns a list of { channel, linkshellName,
-- reason } describing what was dropped so the caller can surface a message.
-- Safe to call when offline: a network error returns nil and leaves the
-- pairing list untouched so we don't unpair on a transient outage.
function api.validate_pairings()
    if not api.config or not api.config.pairings or #api.config.pairings == 0 then
        return {}
    end
    local removed = {}
    -- Snapshot tokens up front so concurrent index shifts during removal
    -- don't skip pairings.
    local snapshot = {}
    for _, p in ipairs(api.config.pairings) do
        snapshot[#snapshot + 1] = {
            token          = p.token,
            channel        = p.channel,
            linkshellId    = p.linkshellId,
            linkshellName  = p.linkshellName,
        }
    end

    for _, p in ipairs(snapshot) do
        if p.token and p.token ~= '' then
            local _, _, status = request_with_token('GET', '/api/addon/me', nil, p.token, true)
            if status == 401 or status == 403 then
                purge_pairing_by_token(p.token)
                removed[#removed + 1] = {
                    channel        = p.channel,
                    linkshellName  = p.linkshellName,
                    linkshellId    = p.linkshellId,
            ...
            -- Network errors (status == nil) and other non-2xx codes are
            -- intentionally ignored. We only purge on an explicit unauthorized
            -- response so a flaky connection doesn't wipe pairings.
        end
    end
    return removed
end
...
-- Pushes a /lsm now alliance snapshot to the server. The snapshot table is
-- the return value of attendance.list_alliance_snapshot(); we convert each
-- entry to the API's expected shape (characterName / mainJob / mainJobLevel
-- / subJob / subJobLevel / zone) here so the addon code can stay readable.
-- Server scopes the snapshot to the addon token's linkshell; no eventId.
function api.post_attendance_snapshot(snapshot)
    if type(snapshot) ~= 'table' then
        return nil, 'Invalid snapshot.'
    end
    local entries = {}
    for _, e in ipairs(snapshot.entries or {}) do
        entries[#entries + 1] = {
    ...
    return request('POST', '/api/addon/attendance-snapshots', {
        capturedAtUtc           = snapshot.capturedAtUtc,
        capturedByCharacterName = snapshot.capturedBy,
