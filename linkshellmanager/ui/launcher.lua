...
function M.draw(is_open, state, callbacks)
    ...
    if imgui.Begin('Att', openPtr) then
        ...
        if state.launcherCompact then
            ...
            -- Suppressed for HNM-style (windowed) events: attendance is
            -- credited per Post Window, not by continuous presence, so
            -- "on break" tracking has no effect on what gets posted.
            if (state.windowMax or 1) <= 1 then
            ...
            attendance_p.draw(state, callbacks)
            -- Hide the Break Room panel for HNM-style (windowed) events.
            -- HNMs award DKP at Post Window time, not via continuous
            -- presence, so "on break / pending" tracking is irrelevant
            -- and the empty panel was just visual noise between the
            -- attendance roster and the Post button.
            if (state.windowMax or 1) <= 1 then
                break_room.draw(state, callbacks)
            end
